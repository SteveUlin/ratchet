...
```
ratchet glean --all --limit 200 --warm-base        # a fork-mode tick: real drain AND A/B data (done-key glean/5-fork)
ratchet glean --pilot-report                        # $0: fold every marker into the fork-vs-oneshot verdict
```

**Pilot method — the drain IS the experiment (ADR-0035).** You never re-glean a chunk to measure: every `--warm-base` tick is real forward progress whose stored markers also feed the A/B. `--pilot-report` folds all glean markers into fork-vs-oneshot cohorts, **stratified by structural score** (the same pointer-only signal the queue orders by) so the greedy drain — richest chunks first, so fork chunks glean later and look poorer — doesn't get blamed on warm-base; it reports cost/chunk, events/chunk, the `input`→`cache_read` token shift (the mechanism), and a clause-by-clause verdict against the flip-to-default rule (≥50 fork chunks, adjusted yield ≥ 0.85, cost ratio ≤ 0.70). The real unit is **chunks per rate-limit window** — the token bucket is shared with your interactive session. Two caveats the report prints: the once-per-tick warm-base call writes no marker (so fork cost/chunk is optimistic on small ticks), and a fork error writes no marker (read `errored` from the run summary for reliability). Keep running fork ticks until the rule trips; that printout is also the before/after you'd show for a resource ask.
