...
class Completion:
    ...
    `cache_*_tokens` are the PROMPT-CACHE evidence (ADR-0035). A cross-invocation cache read needs a
    ...
    cache_creation_tokens: int = 0    # input WRITTEN into the cache this call (~1.25× price); the warm base pays this


Completer = Callable[[str, str], Completion]  # (system_prompt, user_prompt) -> Completion
# The default binding and the test fakes OPTIONALLY extend this callable with a SESSION-CHAIN capability
# for glean's warm-base fork mode (ADR-0035): `.warm(system, base_user) -> session_id` seats a shared
# context ONCE, and `.fork(system, user, session_id) -> Completion` runs a chunk against a fresh fork of
# it. Duck-typed (a callable that also carries `.warm`/`.fork`), so a fake implements it in two trivial
# methods; a plain oneshot Completer simply lacks them, and glean's `--warm-base` refuses loudly against it.
...
# The slim, TOOL-LESS invocation flags shared by EVERY call type (oneshot / warm / fork). Each ratchet
...
def make_cli_completer(model: str = DEFAULT_MODEL, *, timeout: int = 240, retries: int = 2,
    ...
    CACHING is measured, not assumed. cwd defaults to the data root — a CLAUDE.md-free, byte-stable
    working dir, so nothing per-run perturbs the prefix. But whether the CLI's prompt cache actually
    FIRES across these separate `claude -p` processes is not taken on faith: a cross-invocation cache
    read needs a byte-stable prefix over Haiku's 4,096-token minimum, and the oneshot path's shared
    prefix (flags + system prompt) may fall under it. The returned Completion's `cache_read_tokens` /
    `cache_creation_tokens` (from the envelope's `usage`) are how we KNOW whether it fired (ADR-0035).
    The **warm-base fork mode** (`.warm`/`.fork` below) is the deliberate way to GUARANTEE a large,
    byte-stable, cache-warm base — glean opts into it per `--warm-base`.

    A SESSION-CHAIN capability rides on the returned callable (duck-typed, so a fake implements it in
    two methods): `.warm(system, base_user) -> session_id` runs a FIRST call under a fresh `--session-id`
    whose base user turn seats shared context (expect a trivial ack; the reply is discarded), and
    `.fork(system, user, session_id)` runs a chunk under `--resume <id> --fork-session`, inheriting the
    base's history and reading its prompt cache. CONSTRAINT, easy to miss and load-bearing: CLI flags are
    NOT persisted across a resume, so the fork MUST re-pass the same `--system-prompt` and every slim flag
    BYTE-IDENTICALLY to the warm call — the shared `_TOOLLESS_FLAGS` + same `system` guarantee it here.
    A fresh uuid4 per warm; a `--session-id` is never reused for two warm calls.
    ...
    def _run(argv: list[str], user: str) -> Completion:
        """The retry/throttle loop, parameterized over the FULL argv + stdin. Every call type
        (oneshot / warm / fork) builds its own argv and rides this one resilient runner, so the two
        budgets, the envelope parse, and the cache-field capture live in exactly one place."""
        last = "no attempt"
    ...
    def warm(system: str, base_user: str) -> str:
        """Seat a shared base (ADR-0035): a FIRST call under a fresh session id whose base user turn
        carries the shared context (glean's concept digest). The reply is a discarded ack — the base
        exists to prime a large, byte-stable, cache-warm prefix each later fork reads. Returns the id."""
        session_id = str(uuid.uuid4())          # fresh per warm — never reuse a --session-id
        _run([*common, "--session-id", session_id, "--system-prompt", system], base_user)
        return session_id

    def fork(system: str, user: str, session_id: str) -> Completion:
        """Run one chunk against a FRESH FORK of the warm base (`--resume --fork-session`): it inherits
        the base's system + digest turn and reads its prompt cache, but each fork sees base + its own
        chunk ONLY (never a sibling's turn — the contamination hazard the fork dissolves). The flags
        below are byte-identical to `warm`'s except the resume/fork pair (see the constraint above)."""
        return _run([*common, "--resume", session_id, "--fork-session", "--system-prompt", system], user)
